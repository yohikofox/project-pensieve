...
**Decision:** Architecture **Hybrid** (Option 3)
...
Accès Internet:
  ...
  Domaines:
    - auth.pensine.app → Supabase Cloud
    - api.pensine.app → Homelab NestJS (via tunnel)
    - storage.pensine.app → Homelab MinIO (via tunnel)
...
POST https://api.pensine.app/api/captures/upload-url
...
PUT https://storage.pensine.app/{presigned-path}
Body: audio blob

// 4. Mobile confirme au backend
POST https://api.pensine.app/api/captures/{id}/confirm
```
...
Uptime Checks:
  ...
  - https://api.pensine.app/health (Backend homelab)
  - https://storage.pensine.app/minio/health/live (MinIO homelab)
...
8. ✅ ADR-016 : Hybrid Architecture - Cloud Auth + Homelab Storage

**Total : 26 sous-décisions architecturales documentées**
